...
class WebOutputter:
    ...
    def _process_rules_data(self, results, dataset_name: str) -> Dict[str, Any]:
        ...
        for rule_id, entry in results.by_rule_id.items():
            ...
            # Create rule data with preprocessed status (happens here, not in JavaScript)
    ...
    def _create_rule_data(
        ...
        if rule_keyword == "MAY" and rule_obj.entity_type == "Dataset":
    ...
    def _determine_entity_status(self, requirements: list) -> str:
        ...
        Entity status considers both passed and skipped rules as successful.
        Only failed rules count against the entity's status.
        """
        if not requirements:
            return "not"

        # Count passed and skipped rules as successful
        successful_count = sum(
            1 for req in requirements if req["passed"] or req.get("status") == "skipped"
        )
                                        ...
                                        <div class="column-meta">
                                            ...
                                            <span class="status-badge status-${{entity.status}}">
                                                ${{entity.status === 'fully' ? 'Fully Supported' :
                                                  entity.status === 'partial' ? 'Partially Supported' : 'Not Supported'}}
                                            </span>
                                        </div>
                                    </div>
                                    <div class="requirements">
                        ...
                        <div class="entity-group-content">
                            ${{rules.map(rule => `
                                <div class="rule-item"
                                    ...
                                    ${{!rule.passed && (rule.requirement.errorMessage || rule.errorMessage) ? `<div class="requirement-error">${{rule.requirement.errorMessage || rule.errorMessage}}</div>` : ''}}
                                </div>
                            `).join('')}}
                        </div>
                    </div>
            ...
            document.querySelectorAll('.column-card').forEach(card => {{
                ...
                if (statusBadge) {{
                    ...
                    statusBadge.textContent = entityStatus === 'fully' ? 'Fully Supported' :
                                             entityStatus === 'partial' ? 'Partially Supported' : 'Not Supported';
                }}
        ...
        function calculateEntityStatusFromFilters(card, showMust, showShould, showMay) {{
            ...
            requirements.forEach(req => {{
                ...
                if (shouldInclude) {{
                    ...
                    const passed = icon && icon.classList.contains('passed');
                    const skipped = icon && icon.classList.contains('skipped');
                    filteredRequirements.push({{ passed: passed, skipped: skipped }});
                }}
            }});
