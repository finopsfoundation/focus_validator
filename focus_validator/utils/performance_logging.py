...
class PerformanceTracker:
    ...
    def finish(self, additionalContext: Optional[Dict[str, Any]] = None):
        ...
        contextStr = ", ".join(f"{k}={v}" for k, v in fullContext.items()) if fullContext else ""

        if HAS_PSUTIL and memoryMb > 0:
            perfLogger.info(f"Completed {self.operation} in {duration:.3f}s (memory: {memoryMb:.1f}MB) — {contextStr}")
        else:
            perfLogger.info(f"Completed {self.operation} in {duration:.3f}s — {contextStr}")
