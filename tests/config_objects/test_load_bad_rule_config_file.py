...
class TestLoadBadRuleConfigFile(TestCase):
    ...
    def test_load_schema(self):
        rules = [
            ...
            Rule.load_yaml("samples/rule_configs/valid_rule_config_dimension_metadata.yaml")
        ]

        _, checklist = Rule.generate_schema(
            rules=rules, override_config=None
        )
        self.assertEqual(checklist['FV-D001-0001'].status, ChecklistObjectStatus.PENDING)
        self.assertIsNone(checklist['FV-D001-0001'].error)
        self.assertIsNotNone(checklist['FV-D001-0001'].friendly_name)

        self.assertEqual(checklist['FV-D001'].dimension, "ChargeType")
        ...
        self.assertEqual(checklist['FV-D001'].friendly_name, "Ensures that dimension is of string type.")
        ...
        for errored_file_paths in [
            ...
            self.assertEqual(
                checklist[errored_file_paths].status,
                ChecklistObjectStatus.ERRORED
            )
            self.assertIsNotNone(
                checklist[errored_file_paths].error
            )
            self.assertIsNone(checklist[errored_file_paths].friendly_name)
            self.assertEqual(checklist[errored_file_paths].dimension, "Unknown")

    def test_load_schema_without_valid_dimension_metadata(self):
        rules = [
            Rule.load_yaml("samples/rule_configs/bad_rule_config_empty_file.yaml"),
            Rule.load_yaml("samples/rule_configs/bad_rule_config_missing_check.yaml"),
            Rule.load_yaml("samples/rule_configs/valid_rule_config.yaml"),
        ...
        self.assertEqual(checklist['FV-D001-0001'].status, ChecklistObjectStatus.ERRORED)
        self.assertEqual(
            checklist['FV-D001-0001'].error, "ConfigurationError: No configuration found for dimension."
        )
        self.assertIsNotNone(checklist['FV-D001-0001'].friendly_name)
        self.assertEqual(checklist['FV-D001-0001'].dimension, "ChargeType")
