...
class TestCheckFriendlyName(TestCase):
    def test_default_friendly_name_is_generated(self):
        ...
        model_factory = ModelFactory.create_factory(
            model=Rule, check_friendly_name=None, dimension=random_dimension_name
        )

        for _ in range(1000):  # there is no way to generate all values for a field type
            random_model = model_factory.build()
            if random_model.check == "dimension_required":
                self.assertEqual(
                    random_model.check_friendly_name,
                    f"{random_dimension_name} is a required dimension.",
                )
            elif random_model.check == "check_unique":
                self.assertEqual(
                    random_model.check_friendly_name,
                    f"{random_dimension_name}, requires unique values.",
                )
            elif isinstance(random_model.check, AllowNullsCheck):
                if random_model.check.allow_nulls:
                    self.assertEqual(
                        random_model.check_friendly_name,
                        f"{random_dimension_name} allows null values.",
                    )
                else:
                    self.assertEqual(
                        random_model.check_friendly_name,
                        f"{random_dimension_name} does not allow null values.",
                    )
            elif isinstance(random_model.check, DataTypeCheck):
                self.assertEqual(
                    random_model.check_friendly_name,
                    f"{random_dimension_name} requires values of type {random_model.check.data_type.value}.",
                )
            elif isinstance(random_model.check, ValueInCheck):
                options = ",".join(random_model.check.value_in)
                self.assertEqual(
                    random_model.check_friendly_name,
                    f"{random_dimension_name} must have a value from the list: {options}.",
                )
            else:
